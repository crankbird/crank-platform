...
class KevinPortabilityAnalyzer:
    ...
    def analyze_file(self, file_path: Path) -> list[PortabilityFinding]:
        ...
        try:
            ...
            logger.exception("🦙 Kevin encountered error analyzing {file_path}: {e}")
            findings.append(
                PortabilityFinding(
                    severity="medium",
    ...
    def _analyze_container_runtime(self, file_path: Path, content: str) -> list[PortabilityFinding]:
        ...
        docker_violations = [
            ...
            (
                r"FROM\s+.*",
                "Dockerfile without multi-arch support",
            ...
            (
                ...
                "Use runtime-agnostic variables",
            ),
        ]
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    ...
                    if isinstance(value, str) and any(
                        ...
                        for pattern in ["docker", "localhost", "/usr/", "c:\\"]
                    ):
                        findings.append(
                            PortabilityFinding(
                                severity="medium",
    ...
    def _analyze_network_configuration(
        ...
        network_violations = [
            ...
            (
                ...
                "Use portable service discovery",
            ),
    ...
    def generate_portability_report(self, target_path: Path) -> dict:
        ...
        portability_score = 5.0 if total_weight == 0 else max(0.0, 5.0 - total_weight / 15)
    ...
    def _generate_kevin_recommendations(self, findings: list[PortabilityFinding]) -> list[str]:
        ...
        if any("docker" in f.category for f in findings):
            recommendations.append(
                "🔴 CRITICAL: Remove Docker-specific dependencies and implement runtime abstraction",
            )
        ...
        if any("path" in f.category for f in findings):
            recommendations.append(
                "🟡 HIGH: Use pathlib or os.path for cross-platform file operations",
            )

        if any("network" in f.category for f in findings):
            recommendations.append(
                "🟡 HIGH: Implement service discovery instead of hardcoded endpoints",
            )
...
def main():
    ...
    # Print summary
    print("🦙 KEVIN'S PORTABILITY ASSESSMENT")
    ...
    print(
        f"Compatible Runtimes: {report['compatible_runtimes']}/{len(analyzer.supported_runtimes)}",
    )
    print()
