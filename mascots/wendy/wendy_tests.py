...
class WendySecurityAnalyzer:
    ...
    def analyze_file(self, file_path: Path) -> list[SecurityFinding]:
        ...
        try:
            ...
            logger.exception("🐰 Wendy encountered error analyzing {file_path}: {e}")
            findings.append(
                SecurityFinding(
                    severity="medium",
    ...
    def generate_security_report(self, target_path: Path) -> dict[str, Any]:
        ...
        security_score = 5.0 if total_weight == 0 else max(0.0, 5.0 - total_weight / 10)
    ...
    def _generate_wendy_recommendations(self, findings: list[SecurityFinding]) -> list[str]:
        ...
        if any(f.bobby_tables_risk for f in findings):
            recommendations.append(
                "🔴 CRITICAL: Implement parameterized queries/prepared statements to prevent Bobby Tables attacks",
            )

        if any(f.category == "hardcoded_secrets" for f in findings):
            recommendations.append(
                "🔴 CRITICAL: Move all secrets to environment variables or secure vault",
            )

        if any(f.category == "missing_input_validation" for f in findings):
            recommendations.append(
                "🟡 HIGH: Implement comprehensive input validation and sanitization",
            )

        if any(f.category == "weak_cryptography" for f in findings):
            recommendations.append(
                "🟡 HIGH: Upgrade to strong cryptographic algorithms (AES-256, SHA-256+)",
            )

        if any(f.category == "network_security" for f in findings):
            recommendations.append(
                "🟡 HIGH: Enable TLS verification and use HTTPS for all communications",
            )
...
def main() -> None:
    ...
    # Print summary
    print("🐰 WENDY'S SECURITY ASSESSMENT")
