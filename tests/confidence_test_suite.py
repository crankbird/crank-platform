...
class ConfidenceTestSuite:
        ...
        for category_name, test_method in test_categories:
            ...
            try:
                ...
                if not success:
                    ...
                    logger.error("❌ {category_name} tests failed")
                else:
                    logger.info("✅ {category_name} tests passed")

            except Exception as e:
                overall_success = False
                logger.exception("❌ {category_name} tests crashed: {e}")
                self.failed_tests.append(f"{category_name}: {e!s}")
    ...
    async def test_container_health(self) -> bool:
        ...
        try:
            ...
            for line in result.stdout.strip().split("\n")[1:]:  # Skip header
                if any(svc.container_name in line for svc in SERVICES):
                    if "healthy" in line.lower():
                        ...
                        logger.info("  ✅ {service_name}: Healthy")
                    else:
            ...
            logger.exception("Docker command failed: {e}")
            return False
    ...
    async def test_https_security(self) -> bool:
        ...
        for service in SERVICES:
            logger.info("  Testing {service.name} security...")
            ...
            if not (http_blocked and https_works):
                all_secure = False
                logger.error("    ❌ {service.name} security check failed")
            else:
                logger.info("    ✅ {service.name} properly secured")
    ...
    async def test_http_blocked(self, service: ServiceConfig) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                logger.warning("    ⚠️ HTTP not blocked for {service.name} (got response)")
                return False
            ...
            logger.info("    ✅ HTTP properly blocked for {service.name}")
            return True
        except Exception as e:
            # Server disconnected without response is also good - HTTP blocked
            if "disconnected" in str(e).lower() or "protocol" in str(e).lower():
                logger.info("    ✅ HTTP properly blocked for {service.name} (connection refused)")
                return True
            logger.warning("    ⚠️ HTTP test unclear for {service.name}: {e}")
            return False
        ...
        try:
            ...
            async with httpx.AsyncClient(verify=ssl_context, timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    logger.info("    ✅ HTTPS accessible for {service.name}")
                    return True
                logger.error("    ❌ HTTPS failed for {service.name}: HTTP {response.status_code}")
                return False

        except Exception:
            logger.exception("    ❌ HTTPS test failed for {service.name}: {e}")
            return False
    ...
    async def test_ca_certificate_endpoints(self, ca_service: ServiceConfig) -> bool:
        ...
        try:
            ...
            logger.exception("    CA endpoint test failed: {e}")
            return False
    ...
    async def test_certificate_chain(self) -> bool:
        ...
        try:
            ...
            logger.exception("Certificate chain test failed: {e}")
            return False
    ...
    async def get_service_certificate_info(self, service: ServiceConfig) -> bool:
        ...
        try:
            ...
            if "Certificate chain" in result.stdout or "Server certificate" in result.stdout:
                logger.info("    ✅ {service.name} has valid certificate")
                return True
            logger.warning("    ⚠️ {service.name} certificate unclear")
            return True  # Don't fail on this

        except subprocess.TimeoutExpired:
            logger.warning("    ⚠️ {service.name} certificate check timeout")
            return True  # Don't fail on timeout
        except Exception:
            logger.warning("    ⚠️ {service.name} certificate check failed: {e}")
            return True  # Don't fail on this
        ...
        try:
            ...
            async with httpx.AsyncClient(verify=ssl_context, timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return True
                logger.error("    ❌ Platform service failed: HTTP {response.status_code}")
                return False

        except Exception:
            logger.exception("    Platform integration test failed: {e}")
            return False
    ...
    async def test_service_discovery(self) -> bool:
        ...
        if success:
            logger.info("    ✅ All {len(SERVICES)} services discoverable")
        else:
            logger.error("    ❌ Only {reachable_count}/{len(SERVICES)} services discoverable")
    ...
    async def test_api_endpoints(self) -> bool:
        ...
        async with httpx.AsyncClient(verify=ssl_context, timeout=10.0) as client:
            for service in SERVICES:
                logger.info("  Testing {service.name} API...")
                ...
                if not health_success:
                    ...
                    logger.error("    ❌ {service.name} API tests failed")
                else:
                    logger.info("    ✅ {service.name} API tests passed")
            ...
            if response.status_code == 200:
                try:
                    ...
                    if isinstance(data, dict) and data.get("status") in ["healthy", "ok"]:
                        logger.info("    ✅ {service.name} health API working")
                        return True
                except json.JSONDecodeError:
                    # Health endpoint might return plain text
                    if "healthy" in response.text.lower() or "ok" in response.text.lower():
                        logger.info("    ✅ {service.name} health API working (text)")
                        return True

                logger.info("    ✅ {service.name} health endpoint responding")
                return True
            logger.error("    ❌ {service.name} health API failed: HTTP {response.status_code}")
            return False

        except Exception:
            logger.exception("    ❌ {service.name} health API error: {e}")
            return False
    ...
    def generate_final_report(self, overall_success: bool):
        ...
        if overall_success:
            ...
            if self.failed_tests:
                ...
                for _failure in self.failed_tests:
...
async def main():
    ...
    try:
        ...
        logger.exception("💥 Test suite crashed: {e}")
        sys.exit(1)
