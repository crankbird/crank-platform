...
def vulnerable_command_execution(user_input):
    ...
    result = subprocess.run(cmd, check=False, shell=True)  # Very dangerous!
...
def vulnerable_file_operations(filename):
    ...
    Path("/uploads") / filename  # Could be ../../etc/passwd

    # Direct string concatenation in file operations
    with open("/data/" + filename) as f:  # Path traversal risk
        return f.read()
...
class MassiveGodClass:
    ...
    def method16(self):
        pass  # This should trigger Oliver's God Object detection
