...
async def demo_single_mascot_ai_review(
    ...
    print(f"\n{'=' * 80}")
    print("🎭 SINGLE MASCOT AI REVIEW DEMO")
    print(f"{'=' * 80}")
    ...
    print(
        f"🤖 Invoking AI agent with {orchestrator.get_mascot_emoji(mascot)} {mascot.value.title()}'s prompt...",
    )
...
async def demo_collaboration_ai_review(
    ...
    print(f"\n{'=' * 80}")
    print("🤝 MASCOT COLLABORATION AI REVIEW DEMO")
    print(f"{'=' * 80}")
    ...
    avg_confidence = sum(response["confidence"] for response in ai_responses.values()) / len(
        ai_responses,
    )
...
def demo_prompt_customization():
    ...
    print(f"\n{'=' * 80}")
    print("🎨 PROMPT CUSTOMIZATION DEMO")
    print(f"{'=' * 80}")
    ...
    for context in contexts:
        ...
        for mascot in [MascotType.WENDY, MascotType.KEVIN]:
            ...
            context_line = (
                prompt_lines[-1] if prompt_lines[-1].startswith("Context:") else "No context added"
            )
