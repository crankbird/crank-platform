...
Handles certificate lifecycle for workers and controllers.
Workers retrieve certificates from controller.
Controllers have CA privileges for certificate generation.

ALL COMPONENTS USE HTTPS WITH mTLS - NO HTTP CAPABILITY.
"""
...
class CertificateManager:
    ...
    ALL WORKERS USE HTTPS WITH mTLS - NO HTTP FALLBACK.
    """
    ...
    async def retrieve_certificates_from_controller(
        ...
        Args:
            controller_url: Controller HTTPS endpoint
        ...
        if self.certificates_exist():
            logger.info("✅ Certificates found for worker %s", self.worker_id)
            return True

        # Future: Request certificates from controller via HTTPS
        # For now, we expect certificates to be pre-provisioned
        logger.warning(
            "⚠️  Certificate retrieval not yet implemented. "
            "Expected certificates in %s",
            self.cert_dir,
        )
        return False
...
def load_certificates_sync(worker_id: str) -> dict[str, str]:
    ...
    if not cert_manager.certificates_exist():
        logger.error(
            "❌ Certificates not found for worker %s. Expected in: %s",
            worker_id,
            cert_manager.cert_dir,
        )
        raise FileNotFoundError(
            f"Worker certificates not found in {cert_manager.cert_dir}. "
            "HTTPS requires valid certificates - no HTTP fallback available."
        )

    logger.info("🔒 Loaded certificates for worker %s", worker_id)
