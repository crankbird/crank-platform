...
def update_dockerfile_for_service(service_name: str, manifest: dict):
    ...
    for runtime_file in manifest["dependencies"]["runtime_files"]:
        ...
        old_patterns = [
            rf'COPY {filename}',
            rf'COPY initialize_certificates\.py',  # Old name
    ...
    if "plugin.yaml" in content:
        content = re.sub(
            ...
            rf'COPY services/\1 ./plugin.yaml',
            content
        )
...
def main():
    ...
    print(f"🏗️ Updating Dockerfiles to match build manifests")
    ...
    print(f"🚀 Ready to build with standardized file references")
