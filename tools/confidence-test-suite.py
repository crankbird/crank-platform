...
class CrankConfidenceTestSuite:
    ...
    def log_result(self, service: str, test_name: str, status: str, message: str, 
    ...
    def check_container_health(self, service_key: str) -> bool:
        ...
        if exit_code != 0:
            self.log_result(service_key, "Container Health", "FAIL", 
                          f"Container not found: {stderr}", duration)
            return False
            
        health_status = stdout.strip()
        
        if health_status == "healthy":
            self.log_result(service_key, "Container Health", "PASS", 
                          "Container is healthy", duration)
            return True
        else:
            self.log_result(service_key, "Container Health", "FAIL", 
                          f"Container unhealthy: {health_status}", duration)
            return False
    ...
    def check_https_endpoint(self, service_key: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_result(service_key, "HTTPS Endpoint", "PASS",
                              f"Responded with 200 ({duration:.2f}s)", duration,
                              {"status_code": response.status_code, "response_time": duration})
                return True
            else:
                self.log_result(service_key, "HTTPS Endpoint", "FAIL",
            ...
            duration = time.time() - start_time
            self.log_result(service_key, "HTTPS Endpoint", "FAIL",
                          f"Request failed: {str(e)[:50]}...", duration)
            return False
    ...
    def check_no_http_exposure(self, service_key: str) -> bool:
        ...
        try:
            ...
            self.log_result(service_key, "HTTP Security", "FAIL",
                          f"HTTP accessible (BAD): {response.status_code}", duration)
            return False
            
        except requests.exceptions.RequestException:
            duration = time.time() - start_time
            # Exception means HTTP is properly blocked - GOOD
            self.log_result(service_key, "HTTP Security", "PASS", 
                          "HTTP properly blocked", duration)
            return True
        ...
        try:
            ...
            with socket.create_connection(("localhost", service.port), timeout=10) as sock:
                with context.wrap_socket(sock, server_hostname="localhost") as ssock:
                    ...
                    self.log_result(service_key, "Certificate Chain", "PASS",
                                  f"Valid cert: {subject.get('CN', 'Unknown CN')}", duration,
                                  {"subject": subject, "version": cert.get('version')})
                    return True
                    
        except Exception as e:
            duration = time.time() - start_time
            self.log_result(service_key, "Certificate Chain", "FAIL",
                          f"Certificate error: {str(e)[:50]}...", duration)
            return False
    ...
    def check_service_dependencies(self, service_key: str) -> bool:
        ...
        if failed_deps:
            self.log_result(service_key, "Dependencies", "FAIL", 
                          f"Failed deps: {', '.join(failed_deps)}")
            return False
        else:
            self.log_result(service_key, "Dependencies", "PASS",
    ...
    def test_all_services(self) -> Dict[str, Dict[str, bool]]:
        ...
        test_order = ["cert-authority", "platform", "email-classifier", 
                     "email-parser", "doc-converter", "image-classifier", "streaming"]
    ...
    def generate_report(self, all_results: Dict[str, Dict[str, bool]]) -> Dict:
        ...
        for service_key, results in all_results.items():
            ...
            service_critical_passed = results.get('container_health', False) and results.get('https_endpoint', False)
    ...
    def print_summary(self, report: Dict):
        ...
        print(f"📊 Tests: {summary['passed']} passed, {summary['failed']} failed, {summary['skipped']} skipped")
