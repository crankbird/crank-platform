...
def check_containers():
    ...
    exit_code, stdout, stderr = run_command("docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'")
...
def main():
    ...
    if not check_containers():
        ...
        print("💡 Tip: Start containers with: docker-compose -f docker-compose.development.yml up -d")
        sys.exit(1)
