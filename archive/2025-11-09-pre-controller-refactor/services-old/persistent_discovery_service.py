...
from platform_service import DiscoveryServiceInterface, WorkerInfo
...
class PersistentDiscoveryService(DiscoveryServiceInterface):
    ...
    def __init__(self):
        ...
        self.backend = None
        logger.info("🗄️  Initializing persistent discovery with {self.backend_type} backend")
    ...
    def _determine_backend(self) -> str:
        ...
        logger.warning(
            "⚠️  No persistent storage available, using enhanced in-memory with worker recovery",
        )
    ...
    async def initialize(self):
        ...
        logger.info("✅ Persistent discovery service initialized with {self.backend_type} backend")
    ...
    async def _initialize_redis(self):
        ...
        try:
            ...
            logger.exception("❌ Failed to connect to Redis: {e}")
            raise
    ...
    async def _initialize_file(self):
        ...
        if os.path.exists(workers_path):
            try:
                ...
                logger.info("📂 Loaded existing workers from {workers_path}")
            except Exception:
                logger.warning("⚠️  Failed to load workers file: {e}, starting fresh")
                self.workers = {}
        else:
            self.workers = {}

        logger.info("💾 File-based storage initialized at {workers_path}")
    ...
    async def _register_worker_file(self, worker_info: WorkerInfo) -> bool:
        ...
        self.workers[service_type] = [
            w for w in self.workers[service_type] if w.get("worker_id") != worker_info.worker_id
        ]
    ...
    async def find_worker(self, service_type: str) -> Optional[WorkerInfo]:
        ...
        try:
            ...
            logger.exception("❌ Failed to find worker for {service_type}: {e}")
            return None
    ...
    async def list_workers(self) -> dict[str, list[dict[str, Any]]]:
        ...
        try:
            ...
            logger.exception("❌ Failed to list workers: {e}")
            return {}
    ...
    async def cleanup(self):
        """Cleanup resources."""
        if (self.backend_type == "redis" and self.backend) or (
            self.backend_type == "sqlite" and self.backend
        ):
            await self.backend.close()

        logger.info("🧹 Cleaned up {self.backend_type} backend")
