...
async def health_check():
    ...
    return {
        ...
        "provider": cert_service.get_provider_status()["provider_info"]["provider"]
        if cert_service
...
async def get_provider_config():
    ...
    sanitized_info = {
        ...
        if not any(sensitive in k.lower() for sensitive in ["secret", "token", "password", "key"])
    }
...
def main():
    ...
    if not ca_cert_path.exists() or not ca_key_path.exists():
        ...
        subprocess.run(
            [
                ...
                "-nodes",
                "-subj",
                "/CN=crank-cert-authority/O=Crank Platform/OU=Certificate Authority",
            ],
            check=True,
        )
        ...
        if shared_ca_dir.exists():
            ...
            logger.info(
                f"🔗 Root CA certificate copied with secure permissions: {shared_ca_cert_path} (644)",
            )
            logger.info(
                "🔒 SECURITY NOTE: CA certificate is readable (public key material - safe to read)",
            )
        else:
    ...
    logger.info(
        f"🔒 Starting Certificate Authority Service HTTPS-only on {service_host}:{https_port}",
    )
