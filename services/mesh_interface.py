...
class MeshReceiptSystem:
    ...
    def generate_receipt(
        self, request: MeshRequest, response: MeshResponse, start_time: float,
    ) -> MeshReceipt:
...
class MeshInterface(ABC):
    ...
    def generate_receipt(
        self, request: MeshRequest, response: MeshResponse, auth_context: dict[str, Any],
    ) -> MeshReceipt:
        ...
        return MeshReceipt(
            ...
            output_hash=str(hash(json.dumps(response.result, sort_keys=True)))
            if response.result
            else None,
        ...
        async def mcp_endpoint(message: dict):
            ...
            try:
                ...
                return {
                    ...
                    "error": {
                        "code": -1,
                        "message": f"MCP handling error: {e!s}",
                    },
                }
