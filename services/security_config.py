...
class SecurePlatformService:
    ...
    async def secure_worker_call(
        ...
        if self.environment in [
            "production",
            "development-https",
        ] and not worker_endpoint.startswith("https://"):
            raise HTTPException(
                status_code=500,
    ...
    def generate_complete_dev_certificates(cert_dir: Path):
        ...
        subprocess.run(
            [
                ...
                "-nodes",
                "-subj",
                "/CN=Crank Platform CA/O=Crank Platform/OU=Security",
            ],
            check=True,
            capture_output=True,
        )
        ...
        subprocess.run(
            [
                ...
                "rsa:4096",
                "-keyout",
                str(cert_dir / "platform.key"),
                "-out",
                str(cert_dir / "platform.csr"),
                "-nodes",
                "-subj",
                "/CN=platform/O=Crank Platform/OU=Platform",
            ],
            check=True,
            capture_output=True,
        )
        ...
        subprocess.run(
            [
                ...
                "-out",
                str(cert_dir / "client.csr"),
                "-nodes",
                "-subj",
                "/CN=worker-client/O=Crank Platform/OU=Workers",
            ],
            check=True,
            capture_output=True,
        )
...
def initialize_security(environment: Optional[str] = None) -> AdaptiveSecurityConfig:
