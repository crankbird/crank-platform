...
class CertificateAuthorityClient:
    ...
    def __init__(self, ca_service_url: str):
        ...
        self.ca_cert_path = (
            "/shared/ca-certs/ca.crt"  # STANDARDIZED: Single location for Root CA certificate
        )
    ...
    async def wait_for_ca_service(self, max_wait: int = 30) -> bool:
            ...
            try:
                async with aiohttp.ClientSession(
                    connector=aiohttp.TCPConnector(
                        ssl=False,
                    ),  # Accept self-signed CA cert for development
                ) as session:
                    async with session.get(f"{self.ca_service_url}/health") as response:
                        if response.status == 200:
                            health_data = await response.json()
                            logger.info(
                                f"✅ Certificate Authority Service ready: {health_data.get('provider', 'unknown')}",
                            )
                            return True
        ...
        logger.error("❌ Certificate Authority Service not available after {max_wait} seconds")
        return False
...
async def generate_key_pair_and_csr(service_name: str = "platform") -> tuple[str, str]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if service_name == "platform":
            san_names.extend(["crank-platform", "crank-platform-dev"])
...
"""
        ...
        logger.info("✅ Certificate Signing Request (CSR) generated with SAN: {san_list}")
...
async def main() -> None:
    ...
    os.getenv("CRANK_ENVIRONMENT", "development")
    ...
    try:
        ...
        logger.exception("🔒" + "=" * 60)
        logger.exception("❌ SECURE CERTIFICATE INITIALIZATION FAILED")
        logger.exception("🔒" + "=" * 60)
