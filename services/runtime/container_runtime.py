...
class DockerRuntime(ContainerRuntime):
    ...
    def build(self, dockerfile_path: str, tag: str, context: str = ".") -> bool:
        ...
        try:
            ...
            logger.info(f"🏗️ Built Docker image: {tag}")
            return True
        except subprocess.CalledProcessError as e:
    ...
    def stop(self, container_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Docker stop failed: {e.stderr.decode()}")
            return False
    ...
    def remove(self, container_id: str, force: bool = False) -> bool:
        ...
        try:
            ...
            subprocess.run(cmd, check=True, capture_output=True)
            logger.info(f"🗑️ Removed Docker container: {container_id}")
            return True
        except subprocess.CalledProcessError as e:
            logger.error(f"Docker remove failed: {e.stderr.decode()}")
            return False
    ...
    def inspect(self, container_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Docker inspect failed: {e}")
            return {}
...
class PodmanRuntime(ContainerRuntime):
    ...
    def run(self, image: str, config: Dict[str, Any]) -> str:
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            container_id = result.stdout.strip()
            logger.info(f"🦭 Started Podman container: {container_id}")
            return container_id
        except subprocess.CalledProcessError as e:
            logger.error(f"Podman run failed: {e.stderr}")
    ...
    def build(self, dockerfile_path: str, tag: str, context: str = ".") -> bool:
        ...
        try:
            ...
            subprocess.run(cmd, check=True, capture_output=True)
            logger.info(f"🏗️ Built Podman image: {tag}")
            return True
        except subprocess.CalledProcessError as e:
            logger.error(f"Podman build failed: {e.stderr.decode()}")
            return False
    ...
    def stop(self, container_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"🛑 Stopped Podman container: {container_id}")
            return True
        except subprocess.CalledProcessError as e:
            logger.error(f"Podman stop failed: {e.stderr.decode()}")
            return False
    ...
    def remove(self, container_id: str, force: bool = False) -> bool:
        ...
        try:
            ...
            cmd.append(container_id)
            subprocess.run(cmd, check=True, capture_output=True)
            logger.info(f"🗑️ Removed Podman container: {container_id}")
            return True
        except subprocess.CalledProcessError as e:
            logger.error(f"Podman remove failed: {e.stderr.decode()}")
            return False
    ...
    def logs(self, container_id: str, follow: bool = False) -> str:
        ...
        try:
            ...
            logger.error(f"Podman logs failed: {e.stderr}")
            return ""
    ...
    def inspect(self, container_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Podman inspect failed: {e}")
            return {}
...
class RuntimeManager:
    ...
    def __init__(self, preferred_runtime: str = "auto"):
        ...
        logger.info(f"🦙 Kevin selected runtime: {self.runtime.name}")
    ...
    def _detect_best_runtime(self) -> ContainerRuntime:
        ...
        if self.preferred_runtime != "auto" and self.preferred_runtime in available_runtimes:
            ...
            if runtime.is_available():
                logger.info(f"🎯 Using preferred runtime: {self.preferred_runtime}")
                return runtime
            else:
                logger.warning(f"⚠️ Preferred runtime {self.preferred_runtime} not available, falling back...")
        ...
        for runtime_name in priority_order:
            ...
            if runtime.is_available():
                logger.info(f"🔍 Auto-detected runtime: {runtime_name}")
                return runtime
        
        # No runtime available - Kevin is sad
        available_names = [name for name, runtime in available_runtimes.items() if runtime.is_available()]
        raise RuntimeError(
    ...
    def run_service(self, image: str, **kwargs) -> str:
        ...
        try:
            ...
            logger.info(f"🚀 Service deployed: {image} -> {container_id[:12]}")
            return container_id
    ...
    def build_service(self, dockerfile_path: str, tag: str, context: str = ".") -> bool:
        ...
        try:
            ...
            if success:
                logger.info(f"🏗️ Built service image: {tag}")
            else:
                logger.error(f"❌ Failed to build service image: {tag}")
            return success
        except Exception as e:
...
def create_runtime_manager(preferred: str = None) -> RuntimeManager:
